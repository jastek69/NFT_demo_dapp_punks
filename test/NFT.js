...
describe('NFT', () => {
  ...
  describe('Deployment', () => {
    ...
    it('returns the const mint', async () => {
  ...
  describe('Minting', () => {
    ...
    describe('Success', async () => {
     ...
     // const ALLOW_MINTING_ON = Date.now().toString().slice(0, 10) - 1 //Now NOTE Javascript Error
     const ALLOW_MINTING_ON = "1641260195" // using epochconverter to hard code a date 1 year in the past 
    ...
    describe('Failure', async () => {
      ...
      it('rejects minting before allowed time', async () => {
        const ALLOW_MINTING_ON = new Date('May 26, 2030 18:00:00').getTime().toString().slice(0, 10) // Future Date
        const NFT = await ethers.getContractFactory('NFT')
        nft = await NFT.deploy(NAME, SYMBOL, COST, MAX_SUPPLY, ALLOW_MINTING_ON, BASE_URI)
  ...
  describe('Minting', () => {    
    ...
    let transaction, result, balanceBefore
      ...
      it('deduct contract balance', async () => {
